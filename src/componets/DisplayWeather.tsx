...
console.log(api_Endpoint, api_key)
 
  const fetchCurrentWeather = React.useCallback(async (lat: number, lon: number) => {
    const url = `${api_Endpoint}weather?lat=${lat}&lon=${lon}&appid=${api_key}`;
...
const temperature = Number(weatherData?.main?.temp?.toFixed(0)) -273
